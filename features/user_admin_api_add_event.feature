...
Feature: Creating Issues of a Newsletter 
  ...
  Scenario: Adding an article to an issue (draft) of the newsletter
    Given I send a POST request to "/user_admin_api/v1/issues/:id/articles" with valid attributes
    Then the JSON response headers should set appropriately
    Then the response code should be "200"
    And the JSON response should be an array containing the issue's attributes

  Scenario: Adding two articles to an issue (draft) of the newsletter
    Given I send two POST requests to "/user_admin_api/v1/issues/:id/articles" with valid attributes
    Then the JSON response headers should set appropriately
    Then the response code should be "200"
    And the JSON response should be an array containing the issue's attributes

  Scenario: Adding an event to an issue (draft) of the newsletter
    Given I send a POST request to "/user_admin_api/issues/:id/events" with valid attributes
    Then the JSON response headers should set appropriately
    Then the response code should be "200"
    And the JSON response should be an array containing the issue's attributes

 Scenario: Adding two events to an issue (draft) of the newsletter
    Given I send two POST requests to "/user_admin_api/issues/:id/events" with valid attributes
    Then the JSON response headers should set appropriately
    Then the response code should be "200"
    And the JSON response should be an array containing the issue's attributes
